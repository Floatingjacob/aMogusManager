...
What is your selection?: ");
        ...
        public static void runMod() // Runs mods (duh)
        {
            bool modFound = false;
            string input;
            interaction = false;
        ...
        public static async Task removeMod()
        {
        ...
        public static async Task updater()
        {
            ...
            if (latestVersion > currentVersion)
            {
                ...
                if (OperatingSystem.IsLinux())
                {
                    ...
                    Process.Start(new ProcessStartInfo { FileName = "updater", Arguments = $"{Environment.ProcessId.ToString()}", UseShellExecute = true });
                    Environment.Exit(0);
                }
                else if (OperatingSystem.IsWindows())
                {
                    ...
                    Process.Start(new ProcessStartInfo { FileName = "updater.exe", Arguments = $"{Environment.ProcessId.ToString()}", UseShellExecute = true });
                    Environment.Exit(0);
                }
            }
